...
if ($registerResponse.StatusCode -eq 201) {
    ...
    try {
        ...
        # If it's a redirect, try to follow it
        if ($statusCode -eq 302 -or $statusCode -eq 303) {
            $location = $_.Exception.Response.Headers['Location']
            Write-Host "   [DEBUG] Redirect detected to: $location" -ForegroundColor Yellow
            
            # Try to follow the redirect with the session
            try {
                $redirectResponse = Invoke-WebRequest -Uri $location `
                    -WebSession $session `
                    -UseBasicParsing `
                    -ErrorAction Stop
                ...
                $loginResponse = @{
                    ...
                    Content = $redirectResponse.Content | ConvertFrom-Json -ErrorAction SilentlyContinue
    ...
    if ($loginResponse.StatusCode -eq 200) {
        ...
        $sessionResponse = Invoke-ApiRequest -Url "$baseUrl/api/auth/session" -WebSession $session
        
        if ($sessionResponse.StatusCode -eq 200 -and $sessionResponse.Content) {
            Write-Host "[SUCCESS] Session retrieved successfully!" -ForegroundColor Green
            Write-Host "   User: $($sessionResponse.Content.user.email)"
            Write-Host "   Expires: $($sessionResponse.Content.expires)"
        } else {
            Write-Host "[ERROR] Failed to get session" -ForegroundColor Red
            if ($sessionResponse.Error) {
                Write-Host "   Error: $($sessionResponse.Error | ConvertTo-Json -Depth 5)"
            }
        }
    } else {
        Write-Host "[ERROR] Login failed with status $($loginResponse.StatusCode)" -ForegroundColor Red
        if ($loginResponse.Error) {
            Write-Host "   Error: $($loginResponse.Error | ConvertTo-Json -Depth 5)"
        }
    }
} else {
    Write-Host "[ERROR] Registration failed with status $($registerResponse.StatusCode)" -ForegroundColor Red
    if ($registerResponse.Error) {
        Write-Host "   Error: $($registerResponse.Error | ConvertTo-Json -Depth 5)"
    }
}
